...
//	var self = this;
//    if (self.is_thumbnail) { self.data = new Buffer(self.image_data.split(",")[1], 'base64'); }
//	var options = {
...
//		  'X-Upload-Content-Length': self.data.length,
//            //TODO FIX THIS
//		  'X-Upload-Content-Type': (self.is_thumbnail) ? self.image_data.split(":")[1].split(";")[0] : ""
//		},
...
//		  'Content-Length':	self.data.length - self.byteCount,
//		  'Content-Type': (self.is_thumbnail) ? self.image_data.split(":")[1].split(";")[0] : ""
//		}
//	};
//	try {
...
//		  'Authorization':	'Bearer ' + self.tokens.access_token,
//		  'Content-Length':	0,
//		  'Content-Range':	'bytes */' + self.data.length
//		}
//	};
...
//                    progress: ~~((parseInt(response.headers.range.substring(8, response.headers.range.length)) * 100) / parseInt(self.data.length)) });
//				if (response.headers.range == self.data.length) {
