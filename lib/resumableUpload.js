...
resumableUpload.prototype.initUpload = function() {
    console.log("Started...");
	var self = this;
    self.content_type = (self.is_thumbnail) ? self.image_data.split(":")[1].split(";")[0] : self.video_data.original.type;

    if (self.is_thumbnail) { self.data = new Buffer(self.image_data.split(",")[1], 'base64'); }
    else { self.data = self.video_data.createReadStream(); }
    self.filesize = (self.is_thumbnail) ? self.data.length : self.video_data.original.size;
...
resumableUpload.prototype.putUpload = function() {
	...
	try {
        var stream = (self.is_thumbnail) ? resumer().queue(self.data).end() : self.data;
